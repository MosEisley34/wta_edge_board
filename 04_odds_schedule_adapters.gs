function stageFetchOdds(runId, config, fetchWindow, opts) {
  ...
  const rows = classifiedEvents.map((event) => ({
    ...
    opening_lag_fallback_exemption_max_age_minutes: Number.isFinite(Number(event.opening_lag_fallback_exemption_max_age_minutes)) ? Number(event.opening_lag_fallback_exemption_max_age_minutes) : '',
    opening_lag_fallback_minutes_to_start: Number.isFinite(Number(event.opening_lag_fallback_minutes_to_start)) ? Number(event.opening_lag_fallback_minutes_to_start) : '',
